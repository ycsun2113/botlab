...
// static bool enable_pwm_lpf = false;
...
bool mbot_loop(repeating_timer_t *rt)
{
    ...
    if (global_comms_status == COMMS_OK)
    {
        if(drive_mode == MODE_MOTOR_VEL_OL){
            ...
            mbot_ctlr(mbot_vel_cmd, mbot_vel, &mbot_motor_vel_cmd);

            mbot_motor_vel_cmd.velocity[MOT_L] *= params.motor_polarity[MOT_L];
            mbot_motor_vel_cmd.velocity[MOT_R] *= params.motor_polarity[MOT_R];

            mbot_motor_vel.velocity[MOT_L] *= params.motor_polarity[MOT_L];
            mbot_motor_vel.velocity[MOT_R] *= params.motor_polarity[MOT_R];
            ...
            mbot_motor_pwm.utime = global_utime;
            mbot_motor_pwm_cmd.pwm[MOT_R] = _calibrated_pwm_from_vel_cmd(vel_right_comp, MOT_R) + mbot_motor_pwm_cmd.pwm[MOT_R];
            mbot_motor_pwm_cmd.pwm[MOT_L] = _calibrated_pwm_from_vel_cmd(vel_left_comp, MOT_L) + mbot_motor_pwm_cmd.pwm[MOT_L];
        }else {
...
int mbot_calculate_diff_body_vel_imu(float wheel_left_vel, float wheel_right_vel, serial_mbot_imu_t imu, serial_twist2D_t *mbot_vel){
    ...
    mbot_vel->vx =  DIFF_WHEEL_RADIUS * (wheel_left_vel - wheel_right_vel) / 2.0f;
    mbot_vel->vy = 0;
    float origin_wz =  DIFF_WHEEL_RADIUS * (-wheel_left_vel - wheel_right_vel) / (2.0f * DIFF_BASE_RADIUS);
    float dt = MAIN_LOOP_PERIOD;

    float imu_wz = imu.gyro[2];
    
    double delta_GO = imu_wz - origin_wz;
    double thres = 0.005;

    if(abs(delta_GO) > thres){
        mbot_vel->wz = imu_wz;
    }
    else{
        mbot_vel->wz = origin_wz;
    }
